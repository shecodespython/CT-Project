...
# FV
with st.form("FV"):
    st.write(f"Future Value (FV/Cn) - Calculation")
    
    c0 = st.number_input("C0/PV: ")
    n = st.number_input("n: ")
    r = st.number_input("r (%): ")
...
# PV
with st.form("PV"):
    st.write(f"Present Value (PV/C0) - Calculation")
    
    cn = st.number_input("Cn/FV: ")
    n = st.number_input("n: ")
    r = st.number_input("r (%): ")
...
# r
with st.form("r"):
    st.write(f"Discount/Interest Rate (r) - Calculation")
...
with st.form("PV Perpetuity"):
    st.write(f"Present Value (PV/C0) of a Perpetuity - Calculation")
...
with st.form("C Perpetuity"):
    st.write(f"Yearly amount (C) of a Perpetuity - Calculation")
...
with st.form("r Perpetuity"):
    st.write(f"Interest/Discount Rate (r) of a Perpetuity - Calculation")
...
with st.form("PV Increasing Annuity"):
    st.write(f"Present Value (PV) of a Increasing Annuity - Calculation")
...
with st.form("C Increasing Annuity"):
    st.write(f"Yearly Value (C) of a Increasing Annuity - Calculation")
...
with st.form("r Increasing Annuity"):
    st.write(f"Interest/Discount Rate (r) of a Increasing Annuity - Calculation")
...
with st.form("g Increasing Annuity"):
    st.write(f"Growth Rate (g) of a Increasing Annuity - Calculation")
...
with st.form("PV Annuity"):
    st.write(f"Present Value (PV) of an Annuity - Calculation")
...
with st.form("C Annuity"):
    st.write(f"Yearly Amount (C) of an Annuity - Calculation")
...
# NPV
with st.form("NPV"):
    st.write(f"Net Present Value (NPV) - Calculation")
    
    i0 = st.number_input("I0: ")
    c0 = st.number_input("C0: ")
    r = st.number_input("r (%): ")
    n = st.number_input("n: ")
    c = []
...
with st.form("NPV variable r"):
    st.write(f"Net Present Value (NPV) with Variable r - Calculation")
    ...
    for i in range(int(n)):
        key = f"f_{i}"
        fi = st.number_input(f"f{i + 1}: ", key=key)
        r.append(fi)
