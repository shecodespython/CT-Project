...
def Topic4Problem1():
    st.markdown('#### 1) Annual Cost-Equivalent Value')
...
def Topic4Problem2():
    st.markdown('#### 2) Average Operating Cost')
...
def Topic4Problem3():
    st.markdown('#### 3) The Optimal Timing of the Investment')
    ...
    with col2:
        present_cash_flow = st.number_input("Enter net cash flow for Year 0", value=1, step=100.0)
    ...
    for i in range(num_periods):
        net_cash_flow = st.number_input(f"Enter net cash flow for Year {i + 1}", value=1, step=100.0)
        net_cash_flows.append(net_cash_flow)


    discount_rate = st.number_input('Enter the discount rate', value=15, step=0.01)
...
def Topic4Problem4():
    st.markdown('#### 4) Variable Capacity Utilization')
